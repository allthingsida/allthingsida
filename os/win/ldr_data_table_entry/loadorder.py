# PEB/_LDR_DATA_TABLE_ENTRY module enumeration in "Load Order"
#
...
def main(peb_ea=0):
    ...
    list_start = peb.Ldr.InLoadOrderModuleList.__at__

    print(f"PEB                           @ {peb.__at__:x}")
    print(f"PEB.Ldr                       @ {peb.Ldr.__at__:x}")
    print(f"PEB.Ldr.InLoadOrderModuleList @ {list_start:x}")

    print('------')

    entry_ea = peb.Ldr.InLoadOrderModuleList.Flink.__at__
    i = 1
    while True:
        ok, entry = tbl_entry_tp.retrieve(entry_ea)
        ...
        entry_ea = get_ptr(entry.InLoadOrderLinks.__at__)
