...
def main(peb_ea=0):
    ...
    list_start = peb.Ldr.InMemoryOrderModuleList.__at__

    print(f"PEB                             @ {peb.__at__:x}")
    print(f"PEB.Ldr                         @ {peb.Ldr.__at__:x}")
    print(f"PEB.Ldr.InMemoryOrderModuleList @ {list_start:x}")

    print('------')

    entry_ea = peb.Ldr.InMemoryOrderModuleList.Flink.__at__
    i = 1
    while True:
        ok, entry = tbl_entry_tp.retrieve(entry_ea - (0x10 if is_64bits else 0x8))
        ...
        entry_ea = get_ptr(entry.InMemoryOrderLinks.__at__)
