...
void alarm_print(const char *a1)
{
  puts(a1);
  exit(0);
}
...
{
  if ( g_flag1 )
    memcpy(g_rsa_key, c_rsa_key1, sizeof(g_rsa_key));

  if ( g_flag2 )
    memcpy(g_rsa_key, c_rsa_key2, sizeof(g_rsa_key));
...
int check_scrambled_password(unsigned __int8 *str)
{
...
int check_turn_over_page(char *user_input)
{
    ...
    return strncmp(user_input, g_enc_turn_over_the_page, strlen(user_input)) == 0;
}
...
void prompt_1()
{
  ...
  strncpy(g_user_input, user_input, 200);
  ...
  char user_key[32] = {};
  strncpy(user_key, g_user_input, 19); //;!
...
void prompt_4()
{
  ...
  char user_key[32] = {};
  strncpy(user_key, g_user_input4, 19); //;!key
  AES_KEY rsa_key;
